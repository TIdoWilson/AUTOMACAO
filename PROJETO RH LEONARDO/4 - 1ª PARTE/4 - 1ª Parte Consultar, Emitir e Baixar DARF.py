...
WAIT_AFTER_SELECIONAR = 2.5
WAIT_MARCAR_TIMEOUT = 20.0

pag.PAUSE = 0.5
pag.FAILSAFE = False

SAVE_DIR_BASE = BASE_DIR / "DARF"
...
def wait_for_marcar_todas(timeout_s: float = WAIT_MARCAR_TIMEOUT) -> bool:
    end = time.time() + timeout_s
    while time.time() < end:
        wnd = uia.WindowControl(Name="Gerenciador de Sistemas")
        if wnd.Exists(1):
            try:
                for name in ("Marcar todas", "Marcar Todas"):
...
def main(
    ...
    skip_organizar: bool = False,
):
    coords = load_coords(COORDS_PATH)

    if post_only:
        if skip_organizar:
            print("[info] skip_organizar ativo: nada a organizar no modo post-only")
            return
    ...
    for grupo in grupos:
        ...
        if not wait_for_marcar_todas():
            time.sleep(WAIT_AFTER_SELECIONAR)
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--skip-organizar",
        action="store_true",
        help="Nao organiza/renomeia PDFs (mantem na pasta DARF do grupo).",
    )
    args = ap.parse_args()
    main(
        ...
        skip_organizar=args.skip_organizar,
    )
