...
def carregar_cadastros():
    ...
    for _, row in df.iterrows():
        ...
        if nome.lower() in {"nome", "name"} or cpf.lower() in {"cpf", "documento"}:
            continue
        registros.append({"nome": nome, "cpf": cpf})

    if not registros:
        raise ValueError("Nenhuma linha válida encontrada (precisa de Nome na coluna A e CPF na coluna B).")
    return registros
