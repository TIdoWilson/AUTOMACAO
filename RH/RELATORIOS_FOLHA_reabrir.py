...
def executar_jobs_no_erp(jobs: Iterable[EmpresaJob]):
    ...
    Recuperação:
    - Se a janela "Folha de Pagamento" sumir/fechar durante o processamento,
      tenta reabrir (Shift+S -> Gerenciador de Sistemas -> clique 800x187 -> Alt+N)
      e recomeça o job atual na empresa/estab corrente.
    """
    jobs = list(jobs)
    if not jobs:
        return

    def _erro_indica_reabertura(exc: Exception) -> bool:
    ...
    win_pyget = ativar_e_maximizar()
    if not win_pyget:
        # tenta recuperar usando o primeiro job como referência
    ...
    while i < len(jobs):
        ...
        try:
            ...
            print(f"[ERRO] Falha ao trocar empresa: {e}")
            traceback.print_exc()
            if _erro_indica_reabertura(e):
                print("[RECUP] Detecção de quebra/fechamento. Reabrindo e recomeçando o job atual...")
                win_pyget = reabrir_folha(job.empresa, job.estabelecimento)
                empresa_atual = job.empresa
                continue
            i += 1
            continue
