...
class EmpresaJob:
    ...
    @property
    def codigo_erp_normalizado(self) -> str:
...
def montar_lista_processamento(csv_path: Path) -> list[EmpresaJob]:
    ...
    def process_row(row: dict):
        ...
        empresa, estab = _parse_codigo_erp(codigo)
        key = (empresa, estab)
        if key not in flags:
...
def gerar_nome_arquivo_resumo_folha(job: EmpresaJob) -> str:
    """Gera nome do arquivo: 1-1 RESUMO DA FOLHA 012024.pdf"""
    mes_ano = obter_mes_ano_anterior_compacto()
    empresa_fmt = str(job.empresa).lstrip('0') or '0'
    estab_fmt = str(job.estabelecimento).lstrip('0') or '0'
    return f"{empresa_fmt}-{estab_fmt} RESUMO DA FOLHA {mes_ano}.pdf"

def gerar_nome_arquivo_relatorio_ferias(job: EmpresaJob) -> str:
    """Gera nome do arquivo: 1-1 RELATORIODEFERIAS 012024.pdf"""
    mes_ano = obter_mes_ano_anterior_compacto()
    empresa_fmt = str(job.empresa).lstrip('0') or '0'
    estab_fmt = str(job.estabelecimento).lstrip('0') or '0'
    return f"{empresa_fmt}-{estab_fmt} RELATORIODEFERIAS {mes_ano}.pdf"
...
def fechar_todas_as_janelas():
    ...
    # Pré-checagem: antes de abrir o menu "Janela", se já existir um "Aviso do Sistema"
    # pendente (de alguma ação anterior), fecha ele com Alt+S duas vezes.
...
def gerar_resumo_folha_mensal(job: EmpresaJob):
    ...
    print(f"\n[LOG] === Gerando Resumo da Folha para {job.codigo_erp_normalizado} ===")
...
def gerar_relatorio_ferias(job: EmpresaJob):
    ...
    print(f"\n[LOG] === Gerando Relatório de Férias para {job.codigo_erp_normalizado} ===")
...
def executar_jobs_no_erp(jobs: Iterable[EmpresaJob]):
    ...
    for job in jobs:
        print(f"\n{'='*60}")
        print(f"[LOG] Job: {job.codigo_erp_normalizado}")
...
def main():
    ...
    for i, job in enumerate(jobs, 1):
        print(f"  {i}. {job.codigo_erp_normalizado} | Resumo: {job.gerar_resumo_folha} | Férias: {job.gerar_relatorio_ferias}")
