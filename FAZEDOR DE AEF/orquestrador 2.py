...
def localizar_arquivo_final(codigo: str) -> Path:
    """
    Arquivo padrao novo:
        BASE_DIR / 'Arquivos' / codigo / f'Balancete_{codigo}.xlsm'

    Mantive compatibilidade com nome antigo:
        BASE_DIR / 'Arquivos' / codigo / f'BALANCETE AEF {codigo}.xlsm'
    """
    pasta_empresa = BASE_DIR / "Arquivos" / codigo

    candidatos = [
        pasta_empresa / f"Balancete_{codigo}.xlsm",
        pasta_empresa / f"BALANCETE AEF {codigo}.xlsm",
        pasta_empresa / f"Balancete_{codigo}.xlsx",
        pasta_empresa / f"BALANCETE AEF {codigo}.xlsx",
    ]
    ...
    # Se nao existir ainda, retorna o nome padrao esperado
    return pasta_empresa / f"Balancete_{codigo}.xlsm"
...
class OrquestradorAEF:
    ...
    arquivo_final = localizar_arquivo_final(codigo)
    ...
    ok = messagebox.askyesno(
        ...
        (
            ...
            f"Arquivo final identificado:\n{arquivo_final}\n\n"
            "Obs.: o Script 3 sera chamado apenas com --empresa."
        ),
    )
    if not ok:
        return
    ...
    def fluxo():
        ...
        self._log(f"Arquivo final resolvido automaticamente: {arquivo_final}")

        if not arquivo_final.exists():
            self._log(f"ERRO: arquivo final nao encontrado: {arquivo_final}")
            self._log(f"Esperado para esta empresa: Arquivos\\{codigo}\\Balancete_{codigo}.xlsm")
            return

        rc3 = self._rodar_comando(
            [
                py,
                str(SCRIPT_3),
                "--empresa",
