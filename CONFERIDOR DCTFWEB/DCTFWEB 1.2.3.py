...
def salvar_em_excel_incremental(basepath, nome_titular, dados_pagina):
    """
    Salva/atualiza a planilha de forma incremental.

    OBS: para o caso do WILSON, vamos sempre usar o arquivo fixo
    EXCEL_REFERENCIA_PATH, independente de CPF ou CNPJ, justamente
    para que tudo caia na mesma planilha e evitar problemas de nome
    de arquivo com quebras de linha / caracteres inválidos.
    """
    if not dados_pagina:
        return None

    # Ignora basepath e nome_titular e usa SEMPRE a planilha fixa:
    ...
    if caminho.exists():
        # Abre para acrescentar (append) nas linhas já existentes
