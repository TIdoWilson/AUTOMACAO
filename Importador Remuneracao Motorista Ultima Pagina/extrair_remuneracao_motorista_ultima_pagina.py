...
def extrair_metadados(linhas: list[str]) -> tuple[str, str, str, str, str]:
...
def extrair_dados_ultima_pagina(pdf_path: Path, matricula_forcada: str = "") -> ResultadoExtracao:
    ...
    return ResultadoExtracao(
        ...
        empregado=empregado,
        cpf=cpf,
        matricula=(matricula_forcada or matricula or "").strip(),
        valores=valores,
    )
...
def montar_preview(resultado: ResultadoExtracao, linhas_txt: list[str]) -> dict:
    ...
    return {
        ...
        "mensagem_matricula": (
            ""
            if resultado.matricula.strip()
            else "Matricula nao encontrada no PDF. Informe a matricula para gerar o TXT final com codigo correto."
        ),
        "itens": itens,
...
def processar_pdf_para_txt(pdf_path: Path, pasta_saida: Path | None = None, matricula_forcada: str = "") -> tuple[Path, Path]:
    ...
    caminho_txt = destino / f"{nome_base}_ultima_pagina.txt"
