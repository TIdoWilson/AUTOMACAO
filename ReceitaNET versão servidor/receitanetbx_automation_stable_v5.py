...
#
# cd RECEITANET
# python .\receitanetbx_automation_stable_v5.py --config .\config_runtime_doc_v5.json --sistema 7 --data-inicio 2025-12-01 --data-fim 2025-12-31
# ----------------------------
...
def main():
    ...
    ap.add_argument("--cnpj", help="Processa apenas um CNPJ (ignora XLSX)")
    ap.add_argument("--ie", help="Inscrição Estadual (opcional, usado com --cnpj)")
    ...
    try:
        ...
        sistema_default = ""
    sistema = (args.sistema or "").strip() or _prompt("Sistema (ex.: 1, 2, 7, 20)", sistema_default)
    if not sistema:
        raise SystemExit("Sistema não informado.")

    # Período: sempre solicitado, pois muda a cada execução
    data_inicio = (args.data_inicio or "").strip() or _prompt("Data início (YYYY-MM-DD)")
    data_fim = (args.data_fim or "").strip() or _prompt("Data fim (YYYY-MM-DD)")
