...
ITEM_FATOR_R              = "Valor Folha - Fator R"
...
def carregar_mapa_por_cnpj() -> dict:
    ...
    for _, row in df.iterrows():
        ...
        if not cnpj_digits:
            continue
        codigo = _to_str(row.get(col_codigo, ""))
        estab  = _to_str(row.get(col_estab, "")) or "1"
        if codigo:
            mapa[cnpj_digits] = {'codigo_erp': codigo, 'estabelecimento': estab}
    return mapa
...
def montar_log_empresas() -> list[str]:
    ...
    for xml in encontrar_xmls(CAMINHO_PASTA_BASE):
        ...
        linhas.append(f"[OK] Código {codigo} Estab {estab} - Pasta: {pasta}")
    return linhas
...
def carregar_resultados_log(caminho_log: str) -> list[tuple[str, str, str]]:
    out = []
    ...
    with open(caminho_log, 'r', encoding='utf-8') as f:
        for linha in f:
            if not linha.startswith('[OK]'):
                continue
            m = re.search(r"^\[OK\]\s+Código\s+(?P<cod>\S+)\s+Estab\s+(?P<estab>\S+)\s+-\s+Pasta:\s+(?P<pasta>.+)$",
                          linha.strip())
            if m:
                out.append((m.group('cod'), m.group('estab'), m.group('pasta')))
    return out
...
def processar_empresa_com_fator(codigo_erp: str, estab: str, pasta_xml: str):
    """
    Função especial para processar empresas que possuem FATOR.
    Adicione aqui a lógica específica necessária.
    """
    print(f"\n[FATOR] Processando empresa {codigo_erp}/{estab} COM FATOR...")
    ...
    ITEM_FATOR_R = wait_until(
        lambda: find_first_by_subname([root, workspace], ITEM_FATOR_R, ("MenuItemControl", "ButtonControl"), 6),
        timeout=5, interval=0.1
    )
    if ITEM_FATOR_R:
    ...
    print(f"[FATOR] ✓ Processamento com fator finalizado para {codigo_erp}/{estab}")
# ===================== UIA Helpers =====================
def normalize(s):
...
def localizar_nfse_dentro_fiscal():
    ...
    # 3) Janela da NFS-e por PREFIXO (aceita qualquer município/UF após)
    janela_nfse = None
...
def wait_first_aviso_or_importar(janela_nfse, fiscal_win, timeout=600, interval=0.25):
    """
...
def main():
    ...
    for codigo_erp, estab, pasta_xml in empresas:
        ...
        janela_nfse = wait_until(localizar_nfse_dentro_fiscal, timeout=25, interval=0.5)
        if not janela_nfse:
        ...
        resultado = wait_first_aviso_or_importar(janela_nfse, fiscal_win, timeout=600, interval=0.25)
        if not resultado:
        ...
        # ====== FATOR R ======
        try:
            processar_empresa_com_fator(pasta_xml)
        except Exception as e:
            print(f"[WARN] Falha ao calcular FATOR R: {e}")
