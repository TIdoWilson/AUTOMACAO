...
def processar_empresa_com_fator(fatorR: str):
    ...
    pag.write(obter_mes_ano_anterior_slash())
    pag.press('tab'); pag.press('f4'); time.sleep(0.2)
    ...
    janela_cpp = wait_until(localizar_cpp_dentro_fiscal, timeout=25, interval=0.5)
    if not janela_cpp:
        raise RuntimeError("Janela de CPP não foi encontrada a tempo.")

    pag.press('tab'); pag.press('tab'); pag.press('enter'); time.sleep(0.2)
    ...
    btn_apuracao, btn_carregar, btn_gravar = encontrar_botoes_nfse(janela_fator)
    if not btn_apuracao:
    ...
    fiscal_win = get_fiscal_from_nfse(janela_fator)
    
    # Espera concorrente: aviso OU importar habilitar (timeout=600)
    resultado = wait_first_aviso_or_importar(janela_fator, fiscal_win, timeout=600, interval=0.25)
    if not resultado:
...
def main():
    ...
    for codigo_erp, estab, fatorR, pasta_xml in empresas:
        ...
        try:
            ...
            if sinal in ("sim", "s", "true", "1"):
                processar_empresa_com_fator(fatorR)
            # Encerra tela NFS-e
            try:
                fechar_todas_as_janelas()
            except Exception as e:
                print(f"[WARN] Não consegui clicar 'Sair' na janela: {e}")
            else:
