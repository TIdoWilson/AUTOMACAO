...
LOG_DIR = r"\\192.0.0.251\Arquivos\DOCUMENTOS ESCRITORIO\RH\AUTOMATIZADO\DET"
...
def escolher_empresa_inicial(empresas):
    """Permite escolher por qual empresa iniciar o processamento.

    Aceita:
      - Enter ou 0: iniciar pela 1ª empresa
      - Número (1..N): iniciar pela empresa correspondente
      - 'lista' ou 'lista <pagina>': mostra empresas (50 por página)
      - Termo: pesquisa por parte do nome ou do CNPJ (com ou sem máscara)

    Retorna o índice (0-based) da empresa inicial.
    """
    n = len(empresas)
    if n <= 0:
        return 0
    ...
    print(f"\nForam carregadas {n} empresas.")
    print("Selecione por qual empresa iniciar:")
    print("  - Enter (ou 0) -> iniciar pela 1ª empresa")
    print("  - Número 1..N  -> iniciar pela empresa correspondente")
    print("  - lista / lista <pagina> -> mostrar empresas (50 por página)")
    print("  - termo        -> pesquisar por nome ou CNPJ\n")

    por_pagina = 50

    while True:
        try:
            escolha = input("Empresa inicial (Enter/0/1..N/lista/termo): ").strip()
        except EOFError:
            # Ambiente não-interativo (ou stdin fechado): segue padrão
            return 0
        ...
        # comando lista
        m = re.match(r"(?i)^lista(?:\s+(\d+))?$", escolha)
        if m:
            pag = int(m.group(1) or "1")
            if pag < 1:
                pag = 1
            ini = (pag - 1) * por_pagina
            fim = min(ini + por_pagina, n)
            if ini >= n:
                ultima = (n - 1) // por_pagina + 1
                print(f"Página inválida. Última página: {ultima}.")
                continue
            print(f"\nLista {ini+1}–{fim} de {n}:")
            for i in range(ini, fim):
                e = empresas[i]
                print(f"  {i+1:>4} - {e.get('empresa','')} ({e.get('cnpj','')})")
            print("")
            continue
        ...
        if escolha.isdigit():
            ...
            if 1 <= idx <= n:
                return idx - 1
            print(f"Número inválido. Use 1..{n} (ou Enter).")
            continue

        # pesquisa por termo
        termo = escolha.lower()
        termo_cnpj = _cnpj_digits(escolha)

        matches = []
        for i, e in enumerate(empresas):
            nome = (e.get("empresa") or "").lower()
            cnpj_raw = (e.get("cnpj") or "")
            cnpj_dig = _cnpj_digits(cnpj_raw)
            if termo in nome:
                matches.append((i, e))
                continue
            if termo_cnpj and termo_cnpj in cnpj_dig:
                matches.append((i, e))
                continue
            if termo and termo in cnpj_raw.lower():
                matches.append((i, e))
                continue

        if not matches:
            print("Nenhuma empresa encontrada com esse termo. Tente novamente.")
            continue

        if len(matches) == 1:
            i, e = matches[0]
            print(f"Encontrada: {i+1} - {e.get('empresa','')} ({e.get('cnpj','')})")
            return i

        print(f"\nEncontradas {len(matches)} correspondências (mostrando até 30):")
        for i, e in matches[:30]:
            print(f"  {i+1:>4} - {e.get('empresa','')} ({e.get('cnpj','')})")
        print("\nDigite o número exato (1..N) para iniciar, ou outro termo para refinar.\n")
...
if __name__ == "__main__":
    with sync_playwright() as p:
        ...
        inicio = escolher_empresa_inicial(empresas)

        total_empresas = len(empresas)


        for idx, empresa_info in enumerate(empresas[inicio:], start=inicio):

            cnpj = empresa_info["cnpj"]

            razao_social = empresa_info["empresa"]

            print(f"\\n=== Empresa {idx+1}/{total_empresas}: {razao_social} ({cnpj}) ===")
